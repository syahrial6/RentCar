...
const Navbar = () => {
  ...
  // 2. Lock body scroll saat menu mobile buka
  useEffect(() => {
    document.body.style.overflow = isOpen ? "hidden" : "unset";
  }, [isOpen]);

  // 3. Deteksi section aktif (FIX utama di sini)
  useEffect(() => {
    // Kalau bukan homepage → pakai pathname
    if (pathname !== "/") {
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          <div
            className={`flex justify-between items-center px-6 py-3 rounded-2xl transition-all duration-700 ${
              scrolled || pathname !== "/"
          ...
          >
            ...
            >
              ...
              <div className="flex flex-col">
                <span className="font-black text-sm tracking-tight uppercase leading-none text-slate-900">
                  Artha <span className="text-[#fe6b10]">Royal</span>
                </span>
                <span className="text-[8px] font-black tracking-[0.5em] text-slate-500 uppercase mt-1">
                  Group
                </span>
              </div>
            </Link>
          ...
          >
            <div className="flex justify-between items-center p-8 border-b border-slate-50">
              <div className="flex items-center gap-3">
                ...
                <span className="font-black text-sm tracking-tighter text-slate-900 uppercase italic">
                  Artha <span className="text-[#fe6b10]">Royal</span>
                </span>
              </div>
              <button
                onClick={() => setIsOpen(false)}
                className="w-12 h-12 flex items-center justify-center bg-slate-100 text-slate-900 rounded-full"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="mt-auto p-8 bg-slate-50 flex items-center justify-between">
              <div>
                <p className="text-slate-400 font-bold uppercase text-[9px] tracking-[0.3em] mb-1">
                  Butuh Bantuan?
                </p>
              ...
              <a
                href="https://wa.me/6283873325988"
                className="w-14 h-14 bg-slate-900 rounded-2xl flex items-center justify-center text-[#fe6b10]"
              >
                <MessageCircle
                  size={24}
                  fill="currentColor"
